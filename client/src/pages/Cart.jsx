...
const Cart = () => {
  ...
  const handleUpdateQuantity=async(productId,type)=>{
      try {
        const res=await axios.put('https://full-stacl-ecommerce.vercel.app/api/v1/cart/update', { productId, type },{
          headers:{
  ...
  const loadCart=async()=>{
    try {
      const res=await axios.get('https://full-stacl-ecommerce.vercel.app/api/v1/cart',{
        headers:{
          Authorization:`Bearer ${accessToken}`
        }
      })
  ...
  const handleRemove=async(productId)=>{
    try {
      const res= await axios.delete('https://full-stacl-ecommerce.vercel.app/api/v1/cart/remove',{
        data:{ productId },
        headers:{
          Authorization:`Bearer ${accessToken}`
        },
